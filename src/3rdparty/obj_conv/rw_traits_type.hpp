...
#endif
static std::string
alias_name_conversion(const std::string &key, const std::string &alias_name)
{
    if (alias_name.empty()) {
...
inline typename std::enable_if<type_num == sizeof...(_type), void>::type for_each_tuple(
    std::tuple<_type...> &, _func_type)
{}

template<std::size_t type_num = 0, typename _func_type, typename... _type>
    inline typename std::enable_if
    < type_num<sizeof...(_type), void>::type for_each_tuple(std::tuple<_type...> &args,
                                                            _func_type            func)
{
...
{
public:
    reflex_exption(const std::string &err_msg)
        : err_msg_(err_msg)
    {}
    ~reflex_exption() {}
    virtual char const *what() const throw() { return err_msg_.c_str(); }
