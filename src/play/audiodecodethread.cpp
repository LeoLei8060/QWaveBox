...
bool AudioDecodeThread::initialize()
{
    // 音频解码线程初始化
    return true;
}

void AudioDecodeThread::setPacketQueue(AVPacketQueue *queue)
{
    m_packetQueue = queue;
}
...
bool AudioDecodeThread::openDecoder(int streamIndex, AVCodecParameters *codecParams)
{
    if (!codecParams) {
        qWarning() << "无效的编解码参数";
        return false;
...
int AudioDecodeThread::getSampleRate() const
{
    return m_codecContext ? m_codecContext->sample_rate : 0;
}

int AudioDecodeThread::getChannels() const
{
    return m_codecContext ? m_codecContext->channels : 0;
}

AVSampleFormat AudioDecodeThread::getSampleFormat() const
{
    return m_codecContext ? m_codecContext->sample_fmt : AV_SAMPLE_FMT_NONE;
}

int64_t AudioDecodeThread::getChannelLayout() const
{
    return m_codecContext ? m_codecContext->channel_layout : 0;
}

void AudioDecodeThread::process()
{
